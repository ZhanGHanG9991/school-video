  ...
  <div>
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-row class="add-dialog-row">
        <el-col :span="3"><div class="dialog-hint">学生姓名：</div></el-col>
        <el-col :span="7">
    ...
    <student-search @inputFilter="filtering" @inputReset="resett"/>
    <el-card shadow="hover" class="box-card">
      <el-row>
      <el-col :span="3">
        <el-button type="primary" @click="new_student"><i class="el-icon-plus">新增学生</i></el-button>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    getStudentData () {
      this.axios.post('/student/findAllStudent')
        .then((response) => {
          if (response.data.code === '0000') {
            this.tableData = []
            for (let i = 0; i < response.data.detail.length; i++) {
              this.tableData.push(response.data.detail[i])
              this.tableData[i].stuSex = this.tableData[i].stuSex === 0 ? '男' : '女'
            }
            functions.showSuccessMessage('更新查询成功')
          } else {
